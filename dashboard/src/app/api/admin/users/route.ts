...
import { createUser, hashPassword, ensureAuthTables } from '../../../lib/auth';
...
// PATCH /api/admin/users  { id, active?, password? }  -> enable/disable or reset password
export async function PATCH(request: Request) {
  if (!(await isOwner(request))) {
    return NextResponse.json({ ok: false, error: 'unauthorized' }, { status: 401 });
  }
  let body: { id?: number; active?: boolean; password?: string } = {};
  try { body = await request.json(); } catch { /* ignore */ }
